...
pub extern "C" fn atypik_init(
    ...
    // The llama.cpp backend can only be initialised ONCE per process. Any second
    // atypik_init (the app rebuilds its pipeline whenever settings are saved or
    // the Textualiser is toggled) would call LlamaBackend::init() again, which
    // fails - the model then appeared to "fail to load". Once an engine exists,
    // reuse it and report success instead of reloading. (A model-path change
    // therefore needs an app restart, which is acceptable: reloading a ~1 GB
    // model on a settings save is undesirable anyway.)
...
fn run_mode(model: &LlamaModel, mode: &mut Mode, input: &str) -> Result<String> {
    ...
    // Faithful + stable for a corrector: greedy always takes the most likely
    // token (no "creative" rephrasing), and the repetition/frequency penalty
    // stops the loop that pure greedy falls into on small models. Any residual
    // bad output is caught by the C# faithfulness guard and replaced by the
    // user's original text, so the tool never sends a mangled message.
    let mut sampler = LlamaSampler::chain_simple([
        LlamaSampler::penalties(256, 1.3, 0.5, 0.0),
        LlamaSampler::greedy(),
    ]);
    let mut output = String::new();
    // sample() indexes into the LAST decoded batch's outputs, NOT the absolute
    // context position. The prompt batch enabled logits only on its final token
    // (batch index last_batch_len - 1); each generated token is then decoded in a
    // fresh 1-token batch whose single output is at batch index 0. Passing the
    // absolute position (~sys_len) asked for logits at a non-output index ->
    // get_logits_ith invalid -> GGML_ASSERT(logits != nullptr) -> abort/crash.
    let mut sample_idx: i32 = last_batch_len as i32 - 1;
    let mut pos = start_pos;
    let mut produced = 0;
    const MAX_NEW: i32 = 256;

    loop {
        let token = sampler.sample(&mode.ctx, sample_idx);
        ...
        sample_idx = 0; // the fresh 1-token batch exposes its logits at index 0
        pos += 1;
        produced += 1;
    }
