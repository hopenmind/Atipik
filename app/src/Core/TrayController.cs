...
public sealed class TrayController : IDisposable
{
    ...
    // Quick typing-speed presets in the tray, mirroring the Settings slider.
    // The tick shows the active factor; changes from either place stay in sync
    // via SpeedState.Changed. HIGHER = FASTER (see SpeedState).
    private static readonly double[] SpeedPresets = { 0.5, 0.75, 1.0, 1.5, 2.0, 3.0, 5.0 };
    private readonly List<(MenuItem item, double factor)> _speedItems = new();
    ...
    public TrayController(MainWindow main)
    {
        ...
        _menu = BuildMenu();
        SyncSpeedChecks(SpeedState.Factor);       // initial tick
        SpeedState.Changed += SyncSpeedChecks;    // keep tick in sync with the slider
        _hIcon = LoadIcon();
    ...
    /// <summary>
    /// "Typing speed" submenu: one entry per preset factor. Selecting one calls
    /// <see cref="SpeedState.Set"/>, which the kinetic sampler reads live on the
    /// next injection - no pipeline rebuild. The modulation is never bypassed;
    /// this only shifts the base pace (see <see cref="SpeedState"/>).
    /// </summary>
    private MenuItem BuildSpeedMenu()
    {
        var parent = new MenuItem { Header = Loc.T("tray.speed") };
        foreach (double factor in SpeedPresets)
        {
            string label = factor.ToString("0.##", CultureInfo.InvariantCulture) + "×";
            if (factor == SpeedState.Default)
                label += "  (" + Loc.T("tray.speed.natural") + ")";

            // IsCheckable stays false: the tick is driven by SpeedState (so a
            // change from the Settings slider is reflected here too), and clicks
            // set the factor rather than toggling a local check state.
            var item = new MenuItem { Header = label };
            double captured = factor;
            item.Click += (_, _) => SpeedState.Set(captured);

            _speedItems.Add((item, factor));
            parent.Items.Add(item);
        }
        return parent;
    }

    /// <summary>Tick the preset matching the active factor (none if it is custom).</summary>
    private void SyncSpeedChecks(double factor)
    {
        if (!_main.Dispatcher.CheckAccess())
        {
            _main.Dispatcher.BeginInvoke(new Action(() => SyncSpeedChecks(factor)));
            return;
        }
        foreach (var (item, f) in _speedItems)
            item.IsChecked = Math.Abs(f - factor) < 0.001;
    }
    ...
    {
        _disposed = true;
        SpeedState.Changed -= SyncSpeedChecks;
        RemoveIcon();
