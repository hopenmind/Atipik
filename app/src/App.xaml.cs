...
public partial class App : Application
{
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        // Take control of native bridge loading before any P/Invoke, so packaged
        // builds do not depend on default probing (which fails on some machines
        // with "DLL not found" or an untrusted-mount-point 0x800701C0).
        Atypik.LLM.NativeLoader.Register();

        // Restore the last output mode (Off / Correction / Poetry).
        ModeState.Set(ModeState.LoadPersisted());

        // Restore the persisted base typing-speed factor.
        SpeedState.Load();
    ...
    /// <summary>
    /// Set the output mode directly (used by the overlay's quick-settings panel).
    /// Mirrors the Ctrl+Shift+M hotkey: persists, swaps the poetry prompt and
    /// shows the mode toast.
    /// </summary>
    public void SetOutputMode(Core.OutputMode mode)
    {
        ModeState.Set(mode);
        ApplyMode(mode);
    }

    private void ApplyMode(Core.OutputMode mode)
    {
        ModeState.Persist();
        ...
        DebugLog.Write("OutputMode -> {0}", mode);
    }
