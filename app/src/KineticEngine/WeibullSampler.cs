...
public sealed class WeibullSampler
{
    ...
    public int SampleDelay(BigramProfile profile)
    {
        ...
        // User "base speed" scaling. Applies uniformly to the whole rhythm, so the
        // Weibull/bigram/typo modulation is fully preserved - only the base pace
        // shifts around it. Higher factor = FASTER (shorter delays).
        raw /= Atypik.Core.SpeedState.Factor;
    ...
    public int SampleCorrectionPause()
        => (int)Math.Clamp(SampleWeibull(BaseScaleSlow, 1.4) / Atypik.Core.SpeedState.Factor, 80, 600);
    ...
    public int SampleResumptionDelay()
        => (int)Math.Clamp(SampleWeibull(BaseScaleMid * 0.8, _shapeK) / Atypik.Core.SpeedState.Factor, 40, 300);
}
