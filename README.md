...
**The optional correction model** is not bundled, to keep the download small. To enable it, open **Settings > Textualiser**, pick a model (its size is shown) and click **Download the correction model**, or drop a GGUF file at `app\src\LLM\textualiser.gguf`. The app is fully functional without it.
...
cd Atipik
dotnet build app\Atypik.csproj -c Release
# optional: rebuild the local LLM bridge (Rust + CMake + MSVC)
# cd app\rust\atypik-llm; cargo build --release; cd ..\..\..
dotnet run --project app\Atypik.csproj -c Release
```
...
# output: app\Atypik-win-x64.zip (Atypik.exe + atypik_llm.dll if Rust was built)
```
...
<p align="center"><img src="app/assets/logo-editeur.png" width="120" alt="Hope 'n Mind SASU"></p>
<p align="center"><sub>SIREN 938 261 310 &middot; contact@hopenmind.com &middot; hopenmind.com</sub></p>
